...
def interpolate_bounding_boxes(data):
    # Filter out rows with negative car_ids (temporary IDs)
    valid_data = [row for row in data if int(float(row['car_id'])) >= 0]
    
    frame_numbers = np.array([int(row['frame_nmr']) for row in valid_data])
...
with open('test_interpolated.csv', 'w', encoding='utf-8-sig', newline='') as file:
